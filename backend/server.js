...
app.get('/api/students/:id/attendance', (req, res) => {
  ...
  const attendance = mockData.attendance.filter(a => String(a.studentId) === req.params.id && (!courseId || String(a.courseId) === String(courseId)));
  res.json(attendance);
});

app.get('/api/students/:id/results', (req, res) => {
  const results = mockData.results.filter(r => String(r.studentId) === req.params.id);
  res.json(results);
});

// Faculty teaching assignments
app.get('/api/faculty/:id/teaching', (req, res) => {
  const assignments = mockData.teachingAssignments.filter(t => String(t.facultyId) === req.params.id);
